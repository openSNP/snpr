...
class Frequency
  ...
  def perform(snp_id)
    ...
    s.user_snps.find_each do |us|
      if s.allele_frequency.key?(us.local_genotype[0].chr)
      ...
      if us.local_genotype.length > 1
        if s.allele_frequency.key?(us.local_genotype[1].chr)
      ...
      if s.genotype_frequency.key?(us.local_genotype.rstrip)
        s.genotype_frequency[us.local_genotype.rstrip] +=  1

      elsif us.local_genotype.length > 1
        if s.genotype_frequency.key?(us.local_genotype[1].chr + us.local_genotype[0].chr)
