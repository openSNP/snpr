class UserPhenotypesController < ApplicationController
  ...
  def check_and_award_new_phenotypes(amount, achievement_string)
    if current_user.phenotype_creation_counter >= amount && UserAchievement.find_by_achievement_id_and_user_id(Achievement.find_by_award(achievement_string).id, current_user.id).nil?
  ...
  def check_and_award_additional_phenotypes(amount, achievement_string)
    if current_user.phenotype_additional_counter >= amount && UserAchievement.find_by_achievement_id_and_user_id(Achievement.find_by_award(achievement_string).id, current_user.id).nil?
